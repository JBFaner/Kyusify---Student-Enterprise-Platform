<x-admin-layout>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h3 class="text-xl font-bold text-gray-900 dark:text-white tracking-tight">Edit User Account</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400">Update information for {{ $user->name }}</p>
            </div>
        </div>
    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-violet-500 bg-gray-50 dark:bg-[#0B0A0F] text-gray-900 dark:text-white transition-shadow @error('name') border-red-500 @enderror">
                            @error('name')
                        ...
                        <div>
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" required class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-violet-500 bg-gray-50 dark:bg-[#0B0A0F] text-gray-900 dark:text-white transition-shadow @error('email') border-red-500 @enderror">
                            @error('email')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <!-- Right Column: Optional Password Update -->
                    <div class="space-y-6">
                        <div class="bg-violet-50/50 dark:bg-violet-900/10 border border-violet-100 dark:border-violet-900/30 rounded-xl p-5 mb-2">
                            <h4 class="text-sm font-semibold text-violet-800 dark:text-violet-300 mb-1 flex items-center">
                                <svg class="w-4 h-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                Password Reset (Optional)
                            </h4>
                            <p class="text-xs text-violet-600/80 dark:text-violet-400/80 leading-relaxed">Leave these fields blank if you do not wish to change the user's current password.</p>
                        </div>

                        <div>
                            <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">New Password</label>
                            <input type="password" name="password" id="password" class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-violet-500 bg-gray-50 dark:bg-[#0B0A0F] text-gray-900 dark:text-white transition-shadow @error('password') border-red-500 @enderror" placeholder="Leave blank to keep current">
                            @error('password')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="password" name="password_confirmation" id="password_confirmation" class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-violet-500 bg-gray-50 dark:bg-[#0B0A0F] text-gray-900 dark:text-white transition-shadow" placeholder="Leave blank to keep current">
                        </div>
                    </div>
                </div>
